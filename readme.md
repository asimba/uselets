...
_<p align="justify">Примечание: сценарии требуется запускать два раза с последующей перезагрузкой после каждого запуска; данный сценарий требует постоянной доработки, так что при его запуске возможны некоторые ошибки, но на общий функционал это никоим образом не влияет.</p>_  
