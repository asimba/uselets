...
_Примечание: в качестве параметра указывается номер открываемого порта; полезно, когда требуется передать файлы по сети без дополнительных настроек и сервисов._  
...
_Примечание: сценарии требуется запускать два раза с последующей перезагрузкой после каждого запуска; данный сценарий требует постоянной доработки, так что при его запуске возможны некоторые ошибки, но на общий функционал это никоим образом не влияет._  
#### Комплексное решение: отключение (снижение активности) Windows Defender (Windows 10), удаление bloatware, отключение телеметрии и автообновлений Windows 10.  
_Требования (зависимости):_ PowerShell, [w10fixes-binary](https://github.com/asimba/uselets/raw/main/tools/w10fixes.sfx/bin/w10fixes-alt-msvc.zip)  
Потребуется выполнить **на свой страх и риск** ```w10fixes-alt-msvc.exe``` из прилагающегося архива, подтвердив выполение в открывшемся окне защиты SmartScreen. После утвердительного ответа на запрос об очистке и паузы в 10-20 секунд будет выдан запрос "Контроля учётных записей", требуется также ответить утвердительно. После этого система будет трижды перезагружена (причём первый раз в безопасном режиме). Между перезагрузками следует воздержаться от совершения каких-либо действий с компьютером. Рекомендуется после завершения работы программы проверить настройки конфиденциальности (по-умолчанию **блокируется** доступ сторонних приложений к камере и микрофону).  
_Примечание: сценарий тестировался на Windows 10 20H2 (версии 10.0.19042.870) со всеми обновлениями на конец марта 2021 года._  
