...
void install_certs(char *src){
  ...
  sprintf(tpath,"%scerts\\ca\\*",dpath);
  if((hFind=FindFirstFileA(tpath,&FindFileData))!=INVALID_HANDLE_VALUE){
    tpath[strlen(tpath)-1]=0;
    while(FindNextFileA(hFind,&FindFileData)!=0){
      if((FindFileData.dwFileAttributes&FILE_ATTRIBUTE_DIRECTORY)==0){
        sprintf(fpath," -f -addstore CA \"%s%s\"",tpath,FindFileData.cFileName);
        ShExecInfo.lpFile=cmd;
        ShExecInfo.lpParameters=fpath;
        if(ShellExecuteExA(&ShExecInfo)!=FALSE){
          WaitForSingleObject(ShExecInfo.hProcess,INFINITE);
...
int main(int argc,char *argv[]){
  ...
  if(is_admin()&&argc==2){
    ...
    if(MessageBoxA(NULL,"Установить корневые сертификаты?","Внимание!",0x00001034)==6){
