...
void install_svccontrol(char *src){
  char dpath[MAX_PATH+1];
  sprintf(dpath,"%s\\",getenv("PROGRAMDATA"));
  unarc(dpath,src);
  char cmd[MAX_PATH+1];
  sprintf(cmd,"%s\\System32\\schtasks.exe",getenv("WINDIR"));
  ...
  ShExecInfo.lpParameters=dpath;
  sprintf(dpath," /create /tn svccontrol /ru system /xml \"%s\\svccontrol\\svccontrol.xml\"",getenv("PROGRAMDATA"));
...
int main(int argc,char *argv[]){
  if(argc>2) return 0;
  if(is_admin()&&argc==2){
    if(strcmp(argv[1],"install")==0) install_svccontrol(argv[0]);
  }
  else{
    if(MessageBoxA(NULL,"Установить утилиту контроля сервиса?","Внимание!",0x00001034)==6){
      SHELLEXECUTEINFOA ShExecInfo;
      ShExecInfo.cbSize=sizeof(SHELLEXECUTEINFOA);
      ShExecInfo.lpDirectory=NULL;
      ShExecInfo.hwnd=NULL;
